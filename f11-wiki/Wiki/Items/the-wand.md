---
tags:
  - item
topics: []
status: seed
created: 2026-08-08
updated: 2026-08-08
sources:
  - Raw/Sources/interviews/faction-eleven-lore-and-design.md
source_count: 1
aliases: []
item_type: signalling device
rarity: unique
cost: ""
mechanics: []
source: "Found in the ancient parts of the Templar Monastery, chapter two"
---

# The Wand

One of the two **ancient devices** the [[templar-will]] finds in the ancient parts of the
[[templar-monastery]] and uses to signal for aid before retreating. **The wand calls the
[[mystics]]** — and the mystics receive it.

**Type:** signalling device · **Rarity:** unique · **Found:** [[templar-monastery]], chapter two

## What It Does

Signals the [[mystics]] across the disconnection. See [[allied-faction-signals]].

That a **wand** is what reaches the mystics follows the pattern: each device is a piece of the
faction it calls. A wand lying in a Celestial monastery is a mystic thing in a place it does not
belong — which is itself the evidence that the realms were once in contact.

## Open

- What it looks like, what it costs to use, and whether it can be used more than once.
- What it looked like to the Celestials who kept it, and whether they knew it was mystic work.

Not the Ancients' work: it is old allied-faction technology — see [[allied-faction-signals]].
