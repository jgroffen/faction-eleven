...
commercial game** as its goal. The engine is decided — Godot, see [[godot-as-engine]]. There is
**no prior code**; the material that exists is lore held in the maintainer's head.

## Overview

The constraints that shape every other decision:
...
- **Design-ahead-of-code.** The design exists in far more detail than the implementation, which
  is empty. The design currently leads the codebase rather than describing it.
